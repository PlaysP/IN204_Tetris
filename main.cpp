...
float speed[21] = { //Pour ajuster la vitesse de chute en fonction du niveau
...
Grid grid;  
tetromino t = tetromino('T', grid); //Le tettromino actuellement en mouvement
int score = 0;
int level = 0;
bool EnableBotPos = true; //Vaut true si on affiche la position d'atterrissage prévue
int rowsRemovedOnce; // Nombre de lignes supprimées lors du dernier placement de tétraminos
int rowsRemovedCounter = 0; // Compteur total de lignes supprimées (modulo 10 pour ajuster le niveau)
...
int main()
{
    ...
    bool fastfall = false; //Vaut true au moment ou le joueur appuie sur espace pour une chute instantanée
    bool grounded = false; //Vaut true si le tétraminos vient d'atterrir
    ...
    Grid advGrid; //la grille de l'adversaire en mode multijoueur
    ...
    if (MultiScreen) { //écran pour choisir entre créer ou rejoindre une partie
        BeginDrawing();
        ClearBackground(BLACK);
    ...
    if (Playing) {
        ...
        //GameOver quand le joueur ou son adversaire a perdu
        ...
        } // Fait tomber le tétraminos à intervalle régulier en fonction du niveau
...
void tetrominoIsGrounded(){ //Vérifie lorsque le tétraminos atterrit
...
char randomTetromino(){ //génére un tétraminos aléatoire (enfin plutôt son caractère représentant la forme)
...
int updateLevel(int level, int* rowsRemovedCounter){ //met à jour le niveau
...
int updateScore(int rowsRemoved,int score, int level){//met à jour le score
    int points;
    switch(rowsRemoved) {
        case 0:
            points = 0;
